...
public class MatingSeasonTest {
    ...
    public void TestEventExecution() {
        ...
        connector.triggerEvent( "hasAll",new MatingSeason());

        assertEquals(3, threeCards.getCards().size());
        assertEquals(CardDatabase.UNIQUE_CARDS - 60, hasAll.getCards().size());
        assertEquals(5, fiveCards.getCards().size());
